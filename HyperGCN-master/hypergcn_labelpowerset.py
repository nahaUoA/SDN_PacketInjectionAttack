...
from data import data_labelpowerset
...
for i in range(len(train_dataset_dirs)):
    ...
    train_datasets[i], trains[i] = data_labelpowerset.load_data(data_arg)
...
for i in range(len(test_dataset_dirs)):
    ...
    test_datasets[i], tests[i] = data_labelpowerset.load_data(data_arg)


# # initialise HyperGCN
from model import model_labelpowerset

HyperGCN = model_labelpowerset.sequential_train(train_datasets, trains, args)
...
#     if i == 0:
#         HyperGCN = model_labelpowerset.train(HyperGCN, train_datasets[i], trains[i], i,  args)
#         continue
#
#     HyperGCN = model_labelpowerset.initialise_from_prev_model(HyperGCN, train_datasets[i], args)
#
...
for i in range(len(test_datasets)):
    ...
    HyperGCN = model_labelpowerset.initialise_from_prev_model(HyperGCN, test_datasets[i], args)
    acc = model_labelpowerset.test(HyperGCN, test_datasets[i], tests[i], args)
