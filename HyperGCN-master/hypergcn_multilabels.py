...
for i in range(len(train_datasets)):
    ...
    if i == 0:
        HyperGCN = model_multilabels.train(HyperGCN, train_datasets[i], trains[i], i, args)
        continue
    ...
    for param_tensor in HyperGCN['optimiser'].state_dict():
        #print(param_tensor, "\t", HyperGCN['optimiser'].state_dict()[param_tensor].shape)
        print("\n")
        print(param_tensor, "\t", HyperGCN['optimiser'].state_dict()[param_tensor])
...
for i in range(len(test_datasets)):
    ...
    HyperGCN = model_multilabels.initialise_from_prev_model(HyperGCN, test_datasets[i], args)
    acc = model_multilabels.test(HyperGCN, test_datasets[i], tests[i], args)
