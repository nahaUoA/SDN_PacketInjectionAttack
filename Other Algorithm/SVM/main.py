...
def loadData(path):
	...
	#df = pd.read_csv(os.path.abspath(os.path.join(os.getcwd(), path)), usecols = [' SrcHost','Fwd IAT Mean', 'Fwd IAT Std', 'Idle Max', 'Fwd IAT Total', 'Fwd IAT Max', 'Idle Std', 'Flow IAT Std', 'Fwd IAT Min', 'Flow IAT Max', 'Label'])
...
def training(df):
	...
	SVM_clasify = svm.SVC(kernel='rbf', C=1, gamma='auto', max_iter=8000)
	print('[Training] SVM')
	start_train = datetime.now()
	print(">>> Start Time: {}".format(start_train))
	SVM_clasify.fit(X_train_fs, Y_train_fs)
	svm_train_score = SVM_clasify.score(X_train_fs, Y_train_fs)
	print(">>> Training Time: {}".format(datetime.now() - start_train))
	print(f'>>> Training Accuracy : {svm_train_score*100.0}')
	print("")

	return X_test_fs, Y_test_fs, SVM_clasify
...
def testing(X_test_fs, Y_test_fs, model):
	...
	labels = [0,1]
	print(classification_report(Y_test_fs, y_pred, labels = labels, target_names=['label0','label1'], digits = 16)) 
