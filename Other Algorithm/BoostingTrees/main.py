...
def training(df):
	...
	GB_clasify = GradientBoostingClassifier(n_estimators=100, learning_rate=1.0, max_depth=MAX_DEPTH, random_state=0)
	print('[Training] GradientBoosting')
	start_train = datetime.now()
	print(">>> Start Time: {}".format(start_train))
	GB_clasify.fit(X_train_fs, Y_train_fs)
	GB_train_score = GB_clasify.score(X_train_fs, Y_train_fs)
	print(">>> Training Time: {}".format(datetime.now() - start_train))
	print(f'>>> Training Accuracy : {GB_train_score*100.0}')
	print("")

	return X_test_fs, Y_test_fs, GB_clasify

def testing(X_test_fs, Y_test_fs, model):
	print('[Testing] GradientBoosting')
	start_predict = datetime.now()
	y_gb_pred = model.predict(X_test_fs)
	print(y_gb_pred)
	df_pred = pd.DataFrame({'y_gb_pred':y_gb_pred })
	...
	df_Y_test_fs = pd.DataFrame({'Y_test_fs':Y_test_fs })
	df_Y_test_fs = df_Y_test_fs.reset_index(drop=True)
	acc_gb = accuracy_score(Y_test_fs, y_gb_pred)
	recall_gb = recall_score(Y_test_fs, y_gb_pred, average="macro")
	precision_gb = precision_score(Y_test_fs, y_gb_pred, average="macro", zero_division=0)
	mse_gb = mean_squared_error(Y_test_fs, y_gb_pred)
	f1score_gb = f1_score(y_gb_pred, Y_test_fs, average='macro')
	print("real attacker:")
	print(f'Number of IP attackers: {df_Y_test_fs.value_counts(sort=False)}')
	print("pred attacker:")
	...
	print(f'- MSE       : {mse_gb}')
	print(f'- F1 Score  : {f1score_gb}')
	labels = [0,1]
	print(classification_report(Y_test_fs, y_gb_pred, labels = labels, target_names=['label0','label1'], digits = 16))
